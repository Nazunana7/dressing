...
with settings_tab:
    ...
    codex_profile = getattr(config, "codex_profile", None)
    codex_model = getattr(config, "codex_model", None)
    st.write("专用 CODEX_HOME：")
    st.code(str(codex_home or "当前 Codex 默认配置"), language=None)
    st.write("Codex Profile：")
    st.code(codex_profile or "未指定", language=None)
    st.write("模型：")
    st.code(codex_model or "使用该 CODEX_HOME 的默认模型", language=None)
